...
var config = require("../../../be/finicity/.finicity.json"); // TODO: dotenv
...
router.get("/", function(req, res, next) {
  request.get("http://localhost:3000/authenticate", function(
    err,
    response,
    token
  ) {
      ...
      request(
        ...
        function(error, response, finicity) {
          if (!error && response.statusCode == 201) {
            console.log(finicity);
            con.query(
              `UPDATE borrowers SET customer = "${
                finicity.id
              }", customerTime = "${finicity.createdDate}" WHERE id = "${
                result[last].id
              }"`,
              function(err, result) {
                if (err) throw err;
...
router.delete("/", function(req, res, next) {
  request.get("http://localhost:3000/authenticate", function(
    ...
    con.query("SELECT * FROM borrowers", function(err, result) {
      ...
      request(
        {
          uri: `https://api.finicity.com/aggregation/v1/customers/${
            result[last].customer
          }`,
          method: "DELETE",
          headers: {
            "Finicity-App-Key": config.appKey,
        ...
        function(error, response, finicity) {
          if (!error && response.statusCode == 204) {
            console.log(finicity);
            con.query(
              `UPDATE borrowers SET customer = NULL, customerTime = NULL WHERE id = "${
                result[last].id
              }"`,
              function(err, result) {
                if (err) throw err;
