...
router.get("/report", function(req, res, next) {
  request.get("http://localhost:3000/authenticate", function(
    ...
    con.query("SELECT * FROM borrowers", function(err, result) {
      ...
      request(
        {
          uri: `https://api.finicity.com/decisioning/v1/consumers/${
            result[last].consumer
          }/reports/${result[last].report}`,
          method: "GET",
          headers: {
      ...
      request(
        ...
        function(error, response, body) {
          if (!error && response.statusCode == 200) {
            ...
            con.query(
              `UPDATE borrowers SET report = "${report.id}", reportTime = "${
                report.createdDate
              }" WHERE id = "${result[last].id}"`,
              function(err, result) {
                if (err) throw err;
